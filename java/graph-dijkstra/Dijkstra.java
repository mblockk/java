// Methods:
// getDijkstraPath() doDijkstra() computeEuclideanDistance() computeAllEuclideanDistances()
...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
      ...
      double eucDist = Math.sqrt((Math.pow((ux-vx), 2) + Math.pow((uy-vy), 2)));
      return eucDist;
  }
  ...
  public void computeAllEuclideanDistances() {
        for (Vertex x: getVertices()){
            for(Edge e: x.adjacentEdges){
            Vertex v = e.target; 
            e.distance = computeEuclideanDistance(x.x, x.y, v.x, v.y);
            }
        }
  }
  ...
  public void doDijkstra(String s) {
    ...
    while(unknowns.size() !=0){
      Vertex minVal = unknowns.get(0);
      for(int i =1; i<unknowns.size(); i++){
        if (minVal.distance > unknowns.get(i).distance)
            minVal = unknowns.get(i);        
      }
      minVal.known = true;      
        for(Edge e: minVal.adjacentEdges){
           if (!e.target.known){
               double cvw = e.distance;
               if( minVal.distance + cvw < e.target.distance){
                   e.target.distance = cvw + minVal.distance;
                   e.target.prev = minVal;
               }
           }
          unknowns.remove(minVal);  
        }  
    }      
  }      
  /**
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    Vertex endPoint = new Vertex(null, 0, 0);
    endPoint = getVertex(t);
    doDijkstra(s);
    List<Edge> thePath = new LinkedList<Edge>();
      if(endPoint.prev != null){

          while(endPoint.prev != null){
              thePath.add(findEdge(endPoint.prev, endPoint));
...
public Edge findEdge(Vertex prev, Vertex current){
